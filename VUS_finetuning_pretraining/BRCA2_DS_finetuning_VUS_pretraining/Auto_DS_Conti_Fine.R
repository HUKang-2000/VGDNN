...
auto_epoch  <- as.numeric(args[6])
hidden_node <- as.numeric(args[7])
gpu         <- as.numeric(args[8])

Sys.setenv("CUDA_VISIBLE_DEVICES" = gpu)

load(file.path("Data",
               paste0(fold, "-th_Transfer_setting_BRCA2_DiseaseSpecific_forNN_30sets_Nafill_standardized.RData")))

d<-paste("x_train_",fold,"_BRCA2",sep="")
t<-paste("y_train_",fold,"_BRCA2",sep="")
...
load_model_weights_tf(
  Regression_model,
  filepath = file.path("VUS_finetuning_pretraining","BRCA2_DS_finetuning_VUS_pretraining", "pretraining_VUS",
                       paste0(fold, "-th_BRCA2_pretraining_VUS_",auto_drop, "-", auto_noise,"_autoencoder.ckpt"))
...
#Fine_tuning
set.seed(22)
load(file.path("VUS_finetuning_pretraining","BRCA2_DS_finetuning_VUS_pretraining", "DS_Par_set_with_pretraining.RData"))
...
save(Total_pr_te, Total_pr_tr, predicted_tr, predicted_te,
     file = file.path("VUS_finetuning_pretraining","BRCA2_DS_finetuning_VUS_pretraining", "result", paste0("BRCA2_DS_", fold, "-", model, "-th_folds.RData")))
