...
load(file.path("Data",
               paste0(fold, "-th_Transfer_setting_MLH1_DiseaseSpecific_forNN_30sets_Nafill_standardized.RData")))

d<-paste("x_train_",fold,"_MLH1",sep="")
t<-paste("y_train_",fold,"_MLH1",sep="")

x_train<-get(d)
x_train<-apply(x_train[,-1],2,as.numeric)
y_train<-get(t)

d<-paste("x_test_",fold,"_MLH1",sep="")
t<-paste("y_test_",fold,"_MLH1",sep="")
...
load_model_weights_tf(
  Regression_model,
  filepath = file.path("VUS_finetuning_pretraining","MLH1_DS_finetuning_VUS_pretraining", "pretraining_VUS",
                       paste0(fold, "-th_MLH1_pretraining_VUS_",auto_drop, "-", auto_noise,"_autoencoder.ckpt"))
...
#Fine_tuning
set.seed(23)
load(file.path("VUS_finetuning_pretraining","MLH1_DS_finetuning_VUS_pretraining", "DS_Par_set_with_pretraining.RData"))
...
save(Total_pr_te, Total_pr_tr, predicted_tr, predicted_te,
     file = file.path("VUS_finetuning_pretraining","MLH1_DS_finetuning_VUS_pretraining", "result", paste0("MLH1_DS_", fold, "-", model, "-th_folds.RData")))
